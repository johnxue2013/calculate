...
public class Calculator {
            ...
            if (operation.getType() == OperationType.VALUE) {
                ...
                if (symbol != null) {
                    ...
                    for (int j = 0; j < numOfSymbol; j++) {
                        values[numOfSymbol - j - 1] = Double.valueOf(stack.pop().getValue());
                    }
    ...
    private void turnToOperations(String[] split, Operation[] operations) {
        ...
        for (int i = 0; i < split.length; i++) {
            if (split[i].matches("-?[0-9]+") || split[i].matches("-?[0-9]+.?[0-9]+")) {// 判断是否是数值
        ...
        for (int i = 0; i < str.length(); i++) {
            ...
            if (c > 57 || c < 48) {// 非数字
                if (c == 46 && lastC <= 57 && lastC >= 48) {// 小数点
                    ...
                    sb.append(c);
                } else if (c == 40 || c == 41 || c == 42 || c == 43 || c == 45 || c == 47) {// 如果是左括号 右括号 乘号 加号 减号 除号
                                                                                            // 则空格隔开，在append到sb里面
    ...
    private void handleStack(Operation operation) {
        ...
        } else if (stack.isEmpty() || stack.lastElement().getValue().equals("(")) {// 栈stack为空，或栈顶运算符为左括号“(”，则直接将此操作符入栈
            stack.push(operation);
